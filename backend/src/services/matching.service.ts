...
// Life-event labels vary across the app (form labels like "Divorce &
// Separation" vs URL slugs like "divorce"), so we normalize first.
...
export async function suggestMentors(lifeEvent: string, limit = 5): Promise<MatchResult> {
  ...
  for (const mentor of rows) {
    ...
    if (mentor.availability) {
      matchScore += 8;
      reasons.push(`Has stated availability (${mentor.availability})`);
    }
    if (mentor.status === LeadStatus.QUALIFIED) {
      matchScore += 5;
      reasons.push("Qualified and not yet assigned");
    }

    matches.push({ mentor, matchScore: Math.min(100, matchScore), reasons });
  }
...
export interface NewMenteeInput {
  name: string;
  email: string;
  lifeEvent: string;
  story: string;
...
export async function upsertMentee(input: NewMenteeInput) {
