// Notifies the team by email when a high-scoring mentor lead comes in.
// Fire-and-forget: a notification failure must never break lead capture.
//
// Env:
...
import type { Qualification } from "./lead-qualification.service";
...
/**
 * Returns true if an email was sent, false if skipped (below threshold / not
 * configured) or failed. Callers should not await-block lead capture on this.
 */
export async function notifyHighScoringLead(lead: MentorLead, q: Qualification): Promise<boolean> {
  ...
  if (lead.status === LeadStatus.NOT_READY) return false; // don't alert on crisis-flagged applicants
