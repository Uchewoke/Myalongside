    ...
    <div className="flex min-h-screen">
      <div className="relative hidden flex-col justify-between overflow-hidden bg-hero-gradient p-12 lg:flex lg:w-5/12">
        ...
        <div className="relative space-y-6">
          <div className="select-none text-6xl leading-none text-white/20">&ldquo;</div>
        ...
        <div className="relative flex flex-wrap gap-3">
          {["🔒 Private & Confidential", "✅ Verified Mentors", "🆓 Free to start"].map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-xs text-stone-300 backdrop-blur-sm"
            >
              {t}
            </span>
          ))}
      ...
      <div className="flex flex-1 flex-col items-center justify-center bg-stone-50 px-4 py-12 sm:px-8">
        ...
        <p className="mt-3 text-center text-[11px] text-stone-400">
          ...
          <Link href="/legal/safety" className="underline transition-colors hover:text-brand-600">
            Crisis resources →
          </Link>
        </p>
      </div>
